...
def count_word(description_text, number_letters):
...
def sort_top(word_dict, top_words):
...
def main():
    while True:
        ...
        number_letters = int(input('Введите количество букв: '))
        top_words = int(input('Сколько слов вывести: '))
        if name == 'newsafr.json':
            top_10 = sort_top(count_word(read_files_json(name), number_letters), top_words)
            for i in top_10.values():
                print(f'{i[0]} : {i[1]}')
        elif name == 'newsafr.xml':
            top_10 = sort_top(count_word(read_files_xml(name), number_letters), top_words)
            for i in top_10.values():
                print(f'{i[0]} : {i[1]}')
        else:
            break
